...
class TestsBattery(unittest.TestCase):
    ...
    def test_set_value(self):
        battery = Battery(env)
        battery.set_value(battery.fields['system_status'], 0.0)
        self.assertEqual(0.0, battery.get_value(battery.fields['system_status']))

    def test_is_input_connected(self):
        battery = Battery(env)
        battery.set_value(battery.fields['input_connected'], 1.0)
        self.assertTrue(battery.is_input_connected())

    def test_is_converter_started(self):
        battery = Battery(env)
        battery.set_value(battery.fields['converter_started'], 1.0)
        self.assertTrue(battery.is_converter_started())

    def test_set_initial_values(self):
        battery = Battery(env)
        battery.set_initial_values()
        self.assertEqual(72.2, battery.get_value(battery.fields['soc']))
