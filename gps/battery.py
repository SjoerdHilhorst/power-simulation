...
class Battery:
    ...
    def print_all_values(self):
        print("active power in: ", self.get_value(self.address['active_power_in']))
        print("reactive power in: ", self.get_value(self.address['reactive_power_in']))
        print("active power out: ", self.get_value(self.address['active_power_out']))
        print("reactive power out: ", self.get_value(self.address['reactive_power_out']))
        print("converter started: ", self.get_value(self.address['converter_started']))
        print("active power out: ", self.get_value(self.address['active_power_out']))
        print("input connected: ", self.get_value(self.address['input_connected']))
        print("system on backup battery: ", self.get_value(self.address['system_on_backup_battery']))
        print("system status: ", self.get_value(self.address['system_status']))
        print("system mode: ", self.get_value(self.address['system_mode']))
        print("accept values: ", self.get_value(self.address['accept_values']))
        print("active power converter: ", self.get_value(self.address['active_power_converter']))
        print("reactive power converter: ", self.get_value(self.address['reactive_power_converter']))
        print("soc: ", self.get_value(self.address['soc']))
        print("voltage I1 I2 in: ", self.get_value(self.address['voltage_l1_l2_in']))
        print("voltage I2 I3 in: ", self.get_value(self.address['voltage_l2_l3_in']))
        print("voltage I3 I1 in: ", self.get_value(self.address['voltage_l3_l1_in']))
        print("current I1 in: ", self.get_value(self.address['current_l1_in']))
        print("current I2 in: ", self.get_value(self.address['current_l2_in']))
        print("current I3 in: ", self.get_value(self.address['current_l3_in']))
        print("frequency in: ", self.get_value(self.address['frequency_in']))
        print("voltage I1 I2 out: ", self.get_value(self.address['voltage_l1_l2_out']))
        print("voltage I2 I3 out: ", self.get_value(self.address['voltage_l2_l3_out']))
        print("voltage I3 I1 out: ", self.get_value(self.address['voltage_l3_l1_out']))
        print("current I1 out: ", self.get_value(self.address['current_l1_out']))
        print("current I2 out: ", self.get_value(self.address['current_l2_out']))
        print("current I3 out: ", self.get_value(self.address['current_l3_out']))
        print("frequency out: ", self.get_value(self.address['frequency_out']))
        print('----------------------------------')
    ...
    def get_power_factor_in(self):
        ...
        """
        ap = self.get_value(self.address['active_power_in'])
        rp = self.get_value(self.address['reactive_power_in'])
    ...
    def get_power_factor_out(self):
        ...
        """
        ap = self.get_value(self.address['active_power_out'])
        rp = self.get_value(self.address['reactive_power_out'])
        return ap / math.sqrt(ap * ap + rp * rp)
    ...
    def set_active_power_converter(self):
        ...
        """
        apc = self.get_value(self.address['active_power_in']) - self.get_value(self.address['active_power_out'])
        self.set_value(self.address['active_power_converter'], apc)

    def set_reactive_power_converter(self):
        """
        :return: reactive_power_in - reactive_power_out
        """
        rpc = self.get_value(self.address['reactive_power_in']) - self.get_value(self.address['reactive_power_out'])
        self.set_value(self.address['reactive_power_converter'], rpc)
    ...
    def set_soc(self):
        ...
        """
        prev_soc = self.get_value(self.address['soc'])
        apc = self.get_value(self.address['active_power_converter'])

        # multiply by 1000 to convert from kWh
        new_soc = prev_soc + (apc / (self.max_capacity * 1000)) * 3600
        self.set_value(self.address['soc'], new_soc)
    ...
    def set_voltage_I1_I2_in(self):
        ...
        self.set_value(self.address['voltage_l1_l2_in'], value)
    ...
    def set_voltage_I2_I3_in(self):
        ...
        """
        value = self.random_gaussian_value(400, 3)
        self.set_value(self.address['voltage_l2_l3_in'], value)
    ...
    def set_voltage_I3_I1_in(self):
        ...
        """
        value = self.random_gaussian_value(400, 3)
        self.set_value(self.address['voltage_l3_l1_in'], value)
    ...
    def set_current_I1_in(self):
        ...
        """
        ap = self.get_value(self.address['active_power_in'])
        voltage = self.get_value(self.address['voltage_l1_l2_in'])
        pf = self.get_power_factor_in()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l1_in'], current)
    ...
    def set_current_I2_in(self):
        ...
        """
        ap = self.get_value(self.address['active_power_in'])
        voltage = self.get_value(self.address['voltage_l2_l3_in'])
        pf = self.get_power_factor_in()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l2_in'], current)
    ...
    def set_current_I3_in(self):
        ...
        """
        ap = self.get_value(self.address['active_power_in'])
        voltage = self.get_value(self.address['voltage_l3_l1_in'])
        pf = self.get_power_factor_in()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l3_in'], current)
    ...
    def set_frequency_in(self):
        ...
        self.set_value(self.address['frequency_in'], value)
    ...
    def set_voltage_I1_I2_out(self):
        ...
        """
        value = self.random_gaussian_value(400, 3)
        self.set_value(self.address['voltage_l1_l2_out'], value)
    ...
    def set_voltage_I2_I3_out(self):
        ...
        """
        value = self.random_gaussian_value(400, 3)
        self.set_value(self.address['voltage_l2_l3_out'], value)
    ...
    def set_voltage_I3_I1_out(self):
        ...
        """
        value = self.random_gaussian_value(400, 3)
        self.set_value(self.address['voltage_l3_l1_out'], value)
    ...
    def set_current_I1_out(self):
        ...
        """
        ap = self.get_value(self.address['active_power_out'])
        voltage = self.get_value(self.address['voltage_l1_l2_out'])
        pf = self.get_power_factor_out()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l1_out'], current)
    ...
    def set_current_I2_out(self):
        ...
        """
        ap = self.get_value(self.address['active_power_out'])
        voltage = self.get_value(self.address['voltage_l2_l3_out'])
        pf = self.get_power_factor_out()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l2_out'], current)
    ...
    def set_current_I3_out(self):
        ...
        """
        ap = self.get_value(self.address['active_power_out'])
        voltage = self.get_value(self.address['voltage_l3_l1_out'])
        pf = self.get_power_factor_out()
        current = ap / (math.sqrt(3) * voltage * pf) * 1000  # from kW to W
        self.set_value(self.address['current_l3_out'], current)
    ...
    def set_frequency_out(self):
        ...
        """
        value = self.random_gaussian_value(50, 0.01)
        self.set_value(self.address['frequency_out'], value)
